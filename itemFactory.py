...
class ItemFactory:
    def __init__(self,screen,color,width,height,router_img,line_img):
    ...
    def button_line(self,img,pos_x,pos_y,size,action=None):
        mouse = pygame.mouse.get_pos()
        click = pygame.mouse.get_pressed()
        self.screen.blit(img,(pos_x,pos_y))        
        width,height=size
        if pos_x+width > mouse[0] > pos_x and pos_y+height > mouse[1] > pos_y:
            if click[0] == 1 and action != None:
                #first time clicked should initalize start line point
                #second time clicked should initalize end line point and make a line
                print('line')
    ...
    def update(self):
        ...
        self.button_line(self.line_image_obj,25,150,self.line_image_obj.get_size(),action=self.create_line)